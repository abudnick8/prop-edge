              ...
              {isExpanded && (
                <div className="mt-2.5 pt-2.5 border-t border-white/10 flex flex-col gap-2 text-[11px]">
                  ...
                  <div className="grid grid-cols-2 gap-x-3 gap-y-1.5">
                    ...
                    {gameTime && (
                      <>
                        <span className="text-foreground/50 uppercase text-[9px] tracking-wide">Game Time</span>
                        <span className="font-semibold text-foreground">{new Date(gameTime).toLocaleString(undefined, { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}</span>
                      </>
                    )}
                  </div>
      ...
      >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center flex-wrap gap-1.5 mb-1">
              ...
              {m.sport && m.sport !== "OTHER" && (
                <span className="text-[9px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded-sm bg-white/10 text-foreground/90 border border-white/20">
                  {m.sport}
                </span>
              )}
            </div>
          ...
          {m.isWhaleAlert && (
            <span className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
              ...
              {txData?.source === "clob" && txData.txCount > 0 && (
                <span className="text-[9px] text-foreground/70">({txData.txCount}× via CLOB)</span>
              )}
            </span>
          )}
            ...
            {!histLoading && histData && (
              <span className={`text-[9px] font-bold px-2 py-0.5 rounded border ${
                histData.hasRealData
                  ...
                  : "bg-yellow-500/10 text-amber-400 border-yellow-500/20"
              }`}>
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            ...
            { label: m.priceRating === "overpriced" ? "NO Entry" : "Entry",  val: fmtCents(m.entryPrice), color: m.priceRating === "overpriced" ? "text-red-400" : "text-amber-400" },
            ...
            <div className="flex items-center gap-6 text-xs">
              ...
              {m.crossDelta !== null && m.crossDelta > 5 && (
                <div>
                  ...
                  <p className="font-bold text-amber-400">Buy cheaper side</p>
                </div>
              )}
            </div>
          </div>
...
function MarketCard({ m, onClick }: { m: PredMkt; onClick: () => void }) {
  ...
  return (
    ...
    >
      ...
      {/* Click hint */}
      <div className="absolute top-2 right-2 text-[9px] text-foreground/70 font-medium">tap for chart ›</div>
      ...
      <div className="flex items-start justify-between gap-2 pr-12">
        <div className="flex-1 min-w-0">
          <div className="flex items-center flex-wrap gap-1.5 mb-1">
            ...
            {isToday && <span className="text-[10px] font-bold text-amber-400">⚡ TODAY</span>}
              ...
              return (
                <div className="mt-0.5">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    {hidden > 0 && (
                      <p className="text-[10px] text-foreground/70 mt-0.5 pl-5">+{hidden} more legs · tap to see all</p>
                    )}
                  </div>
                </div>
              );
      ...
      <div className="flex flex-col gap-1.5 pt-1 border-t border-white/5">
        ...
        {m.isWhaleAlert && (
          <div className="flex items-center gap-1.5">
            <span className="text-[9px] text-foreground/70">Purchase pattern:</span>
            <PurchaseTypeBadge vol24h={m.vol24h} source={m.source} smartScore={m.smartScore ?? 0} />
          </div>
        )}
      </div>
      ...
      <div className="mb-5 rounded-xl border border-border overflow-hidden">
        ...
        {legendOpen && (
          <div className="px-4 pt-4 pb-5 grid sm:grid-cols-2 gap-x-6 gap-y-4 bg-card">
            ...
            {/* Entry / Target */}
            <div className="flex gap-3">
              <div className="shrink-0 mt-0.5 w-7 h-7 rounded-lg bg-yellow-500/10 border border-yellow-500/20 flex items-center justify-center text-[10px] font-black text-amber-400">→</div>
              <div>
                <p className="text-[11px] font-black text-amber-400 uppercase tracking-wide mb-0.5">Entry → Target</p>
                <p className="text-[11px] text-muted-foreground leading-relaxed">
            ...
            <div className="flex gap-3">
              ...
              <div>
                <p className="text-[11px] font-black text-amber-400 uppercase tracking-wide mb-0.5">⚡ TODAY</p>
                <p className="text-[11px] text-muted-foreground leading-relaxed">
                  This market closes within 24 hours — it's an active in-play or same-day event. These appear at the top of every list. Use the <span className="text-amber-400 font-semibold">⚡ Today</span> filter to see only these.
                </p>
              </div>
            </div>
