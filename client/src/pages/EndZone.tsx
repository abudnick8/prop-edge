...
// ─── Shared week selector — pick current week or up to 3 weeks ahead ─────────
function WeekDropdown({ weeks, selected, onChange, currentWeek }: {
  weeks: number[]; selected: number; onChange: (w: number) => void; currentWeek?: number;
}) {
  if (!weeks || weeks.length <= 1) return null;
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
      <span style={{ fontSize: 11, fontWeight: 700, color: MUTED }}>NFL Week:</span>
      <select
        value={selected}
        onChange={e => onChange(Number(e.target.value))}
        style={{ fontSize: 12, fontWeight: 800, color: NAVY, background: "#fff",
          border: "1px solid rgba(19,35,58,0.18)", borderRadius: 8, padding: "5px 10px", cursor: "pointer" }}>
        {weeks.map(w => (
          <option key={w} value={w}>
            Week {w}{currentWeek != null && w === currentWeek ? " (current)" : ""}
          </option>
        ))}
      </select>
    </div>
...
function WaiverRadarPanel({ data }: { data: WaiverPlayer[] }) {
  ...
  const [waiverWeek, setWaiverWeek] = useState<number | null>(null);
  const positions = ["ALL", "QB", "RB", "WR", "TE"];
  const filtered = posFilter === "ALL" ? data : data.filter(p => p.position === posFilter);
  const waiverWeeks: number[] = Array.from(
    new Set(data.flatMap((p: any) => (p.upcomingSchedule ?? []).map((u: any) => u.week)))
  ).sort((a, b) => a - b);
  const activeWaiverWeek = waiverWeek ?? waiverWeeks[0] ?? null;
  ...
  return (
    <div>
      ...
      <WeekDropdown weeks={waiverWeeks} selected={activeWaiverWeek ?? waiverWeeks[0]} onChange={setWaiverWeek} currentWeek={waiverWeeks[0]} />
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {filtered.map((p, i) => (
          <div key={i} style={{ background: "#fff", borderRadius: 12, border: "1px solid rgba(19,35,58,0.10)", padding: "12px 14px" }}>
            ...
            {(() => {
              const wkEntry = ((p as any).upcomingSchedule ?? []).find((u: any) => u.week === activeWaiverWeek);
              if (!wkEntry) return null;
              return (
                <div style={{ marginTop: 8, paddingTop: 8, borderTop: "1px solid rgba(19,35,58,0.08)" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
                    <span style={{ fontSize: 9, fontWeight: 700, color: MUTED }}>WEEK {wkEntry.week} MATCHUP</span>
                  ...
                  <div style={{ fontSize: 10, padding: "2px 7px", borderRadius: 8, background: "rgba(19,35,58,0.05)", color: NAVY, display: "inline-block" }}>
                    vs {wkEntry.opponent} <b style={{ color: wkEntry.grade === "A" ? "#16a34a" : wkEntry.grade === "B" ? GOLD_COLOR : wkEntry.grade === "C" ? "#f97316" : "#ef4444" }}>{wkEntry.grade}</b>
                  </div>
                </div>
              );
            })()}
...
function MatchupHeatmapPanel({ data }: { data: MatchupRow[] }) {
  ...
  const heatmapWeeks: number[] = Array.from(
    new Set(data.flatMap((row: any) => (row.upcomingOpponents ?? []).map((u: any) => u.week)))
  ).sort((a, b) => a - b);
  const activeHeatmapWeek = heatmapWeek ?? heatmapWeeks[0] ?? null;
  ...
  return (
    <div>
      ...
      <WeekDropdown weeks={heatmapWeeks} selected={activeHeatmapWeek ?? heatmapWeeks[0]} onChange={setHeatmapWeek} currentWeek={heatmapWeeks[0]} />
            ...
            <div key={row.team}
              ...
              {isOpen && det && (
                <div style={{ borderTop: `1px solid ${c.border}`, padding: "12px 13px", background: "#fff" }}>
                  ...
                  {/* Selected-week outlook — who this team's offense faces in the chosen week */}
                  {(() => {
                    const u = ((row as any).upcomingOpponents ?? []).find((x: any) => x.week === activeHeatmapWeek);
                    if (!u) return null;
                    ...
                    return (
                      <div style={{ marginTop: 10 }}>
                        <div style={{ fontSize: 10, fontWeight: 700, color: MUTED, textTransform: "uppercase",
                          letterSpacing: "0.06em", marginBottom: 5 }}>Week {u.week} Matchup — {row.team} Offense</div>
                        <div style={{ border: `1px solid ${uc.border}`, background: uc.bg, borderRadius: 8,
                          padding: "5px 9px", textAlign: "center", minWidth: 64, display: "inline-block" }}>
...
function FantasyToolsPanel({
  ...
  const [dstWeek, setDstWeek] = useState<number | null>(null);
  const [gameScriptWeek, setGameScriptWeek] = useState<number | null>(null);
  const [startSitWeek, setStartSitWeek] = useState<number | null>(null);
  const sections = [
    ...
    <div>
      ...
      {section === "gamescript" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          />
          {(() => {
            const gsCurrentWeek: number = gameScript?.currentWeek ?? 1;
            const gsForecastWeeks: number[] = (gameScript?.forecast ?? []).map((f: any) => f.week);
            const gsAllWeeks = [gsCurrentWeek, ...gsForecastWeeks];
            const activeGsWeek = gameScriptWeek ?? gsCurrentWeek;
            const isForecastWeek = activeGsWeek !== gsCurrentWeek;
            const forecastEntry = gameScript?.forecast?.find((f: any) => f.week === activeGsWeek);
            const gsGames = isForecastWeek ? (forecastEntry?.games ?? []) : (gameScript?.games ?? []);
            return (
              <>
                <WeekDropdown weeks={gsAllWeeks} selected={activeGsWeek} onChange={setGameScriptWeek} currentWeek={gsCurrentWeek} />
                <p style={{ fontSize: 11, color: MUTED, marginBottom: 6 }}>
                  {isForecastWeek
                ...
                {!isForecastWeek && gsGames.map((g: any, i: number) => (
                  <div key={i} style={{ background: "#fff", borderRadius: 12, border: "1px solid rgba(19,35,58,0.10)", padding: "12px 14px", marginBottom: 10 }}>
                ...
                {isForecastWeek && gsGames.map((g: any, gi: number) => (
                  <div key={gi} style={{ background: "rgba(19,35,58,0.03)", borderRadius: 10, border: "1px solid rgba(19,35,58,0.08)", padding: "10px 12px", marginBottom: 8 }}>
                    <div style={{ fontSize: 13, fontWeight: 800, color: NAVY, marginBottom: 6 }}>{g.away} @ {g.home}</div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                      {[g.awayScript, g.homeScript].map((t: any, ti: number) => (
                        <div key={ti} style={{ fontSize: 11, color: MUTED }}>
                ...
                {isForecastWeek && gsGames.length === 0 && (
                  <p style={{ fontSize: 11, color: MUTED, fontStyle: "italic" }}>No schedule data available yet for this week.</p>
                )}
              </>
            );
          })()}
      ...
      {section === "startsit" && (
        <div>
          ...
          {ssResult && !ssResult.error && (() => {
            const ssWeeks: number[] = (ssResult.p1?.forecast ?? []).map((f: any) => f.week);
            const activeSsWeek = startSitWeek ?? ssWeeks[0] ?? null;
            return (
            <div style={{ marginTop: 14, background: "#fff", borderRadius: 12, border: "1px solid rgba(19,35,58,0.10)", padding: "14px" }}>
              ...
              {ssWeeks.length > 1 && (
                <div style={{ marginBottom: 10 }}>
                  <WeekDropdown weeks={ssWeeks} selected={activeSsWeek ?? ssWeeks[0]} onChange={setStartSitWeek} currentWeek={ssWeeks[0]} />
                </div>
              )}
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                {[ssResult.p1, ssResult.p2].map((p: any, j: number) => {
                  const wkEntry = (p.forecast ?? []).find((f: any) => f.week === activeSsWeek);
                  return (
                  <div key={j} style={{ background: p.name === ssResult.start ? "rgba(34,197,94,0.08)" : "rgba(239,68,68,0.06)",
                    ...
                    {wkEntry && (
                      <div style={{ marginTop: 8, paddingTop: 8, borderTop: "1px solid rgba(19,35,58,0.08)" }}>
                        <div style={{ fontSize: 9, fontWeight: 700, color: MUTED, marginBottom: 4 }}>WEEK {wkEntry.week} OUTLOOK</div>
                        <div style={{ fontSize: 10, padding: "2px 6px", borderRadius: 6, background: "rgba(19,35,58,0.06)", color: NAVY, display: "inline-block" }}>
                          vs {wkEntry.opponent} <b style={{ color: gradeColor(wkEntry.grade) }}>{wkEntry.grade}</b>
                        </div>
                      </div>
                    )}
                  </div>
      ...
      {section === "dst" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          />
          {(() => {
            const dstWeeks: number[] = streamingDST?.forecastWeeks ?? [];
            const activeDstWeek = dstWeek ?? streamingDST?.week ?? dstWeeks[0] ?? null;
            const weekEntry = streamingDST?.forecast?.find((f: any) => f.week === activeDstWeek);
            const dstTeams = weekEntry ? weekEntry.teams : (streamingDST?.teams ?? []);
            return (
              <>
                <WeekDropdown weeks={dstWeeks} selected={activeDstWeek ?? dstWeeks[0]} onChange={setDstWeek} currentWeek={streamingDST?.week} />
                <p style={{ fontSize: 11, color: MUTED, marginBottom: 6 }}>
                  Defenses available on most waiver wires with top matchups in the selected week. Stream the right DST for a huge points edge.
                </p>
                {dstTeams.map((t: any, i: number) => (
                  <div key={i} style={{ background: "#fff", borderRadius: 12, border: "1px solid rgba(19,35,58,0.10)", padding: "12px 14px", marginBottom: 10 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                      <div>
...
function NflPickCard({ pick, label, isRunnerUp = false, isOwner = false, sport = "NFL", onGrade }: {
  ...
  const Section = ({ title, icon, children }: { title: string; icon: string; children: React.ReactNode }) => (
    <div style={{ marginBottom: 12 }}>
      <div style={{ fontSize: 9, fontWeight: 800, color: NAVY, textTransform: "uppercase", letterSpacing: 1, marginBottom: 7,
  ...
  const StatRow = ({ label: lbl, value, highlight }: { label: string; value: string; highlight?: boolean }) => (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center",
  ...
  const TileRow = ({ items }: { items: Array<{ label: string; value: string; color?: string } | null> }) => (
    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
      {(items.filter(Boolean) as Array<{ label: string; value: string; color?: string }>).map((t, i) => (
        <div key={i} style={{ background: "rgba(19,35,58,0.05)", borderRadius: 8, padding: "5px 10px", flex: "1 1 auto", minWidth: 70 }}>
          <div style={{ fontSize: 8, fontWeight: 700, color: MUTED, textTransform: "uppercase", letterSpacing: 0.8 }}>{t.label}</div>
          <div style={{ fontSize: 12, fontWeight: 800, color: t.color ?? NAVY }}>{t.value}</div>
        </div>
      ))}
    </div>
  );
  ...
  return (
    ...
    <div style={{
      ...
      {open && (
        <div style={{ padding: "0 14px 14px", borderTop: "1px solid rgba(19,35,58,0.08)" }}>
          ...
          {(sharp.sharpScore > 0 || sharp.sharpDirection) && (
            <Section title={sharp.isFallback ? "Sharp Money (Est.)" : "Sharp Money"} icon="💰">
              ...
              <div style={{ background: sharp.sharpScore >= 65 ? "rgba(34,197,94,0.06)" : "rgba(19,35,58,0.04)",
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 900, color: sharp.sharpScore >= 65 ? "#16a34a" : NAVY }}>
                  ...
                  <div style={{ marginTop: 8, display: "flex", flexWrap: "wrap", gap: 4 }}>
                    {sharp.sharpSignals.map((sig: string, i: number) => (
                      <span key={i} style={{ fontSize: 9, fontWeight: 700, background: "rgba(19,35,58,0.08)",
                        borderRadius: 6, padding: "2px 7px", color: NAVY }}>{sig}</span>
                    ))}
                  </div>
                )}
              </div>
          ...
          {/* Injuries */}
          {((injuries.pickInjuries && injuries.pickInjuries.length > 0) ||
            (injuries.oppInjuries && injuries.oppInjuries.length > 0)) && (
            <Section title="Injury Report" icon="⚠️">
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {injuries.pickInjuries && injuries.pickInjuries.length > 0 && (
                  <div style={{ background: "rgba(239,68,68,0.05)", borderRadius: 10, padding: "8px 11px" }}>
                    <div style={{ fontSize: 9, fontWeight: 800, color: "#dc2626", textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 4 }}>
                      {pick.pickTeam} Injuries
                    </div>
                    {injuries.pickInjuries.slice(0, 5).map((inj: string, i: number) => (
                      <div key={i} style={{ fontSize: 10, color: MUTED, padding: "2px 0" }}>• {inj}</div>
                    ))}
                  </div>
                )}
                {injuries.oppInjuries && injuries.oppInjuries.length > 0 && (
                  <div style={{ background: "rgba(239,68,68,0.04)", borderRadius: 10, padding: "8px 11px" }}>
                    <div style={{ fontSize: 9, fontWeight: 800, color: "#6b7280", textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 4 }}>
                      Opp Injuries
                    </div>
                    {injuries.oppInjuries.slice(0, 5).map((inj: string, i: number) => (
                      <div key={i} style={{ fontSize: 10, color: MUTED, padding: "2px 0" }}>• {inj}</div>
                    ))}
                  </div>
                )}
          ...
          {/* Home Field */}
          {homeField.pickSide && (
            <Section title="Home Field" icon="🏟️">
              <div style={{ background: "rgba(19,35,58,0.04)", borderRadius: 10, padding: "10px 12px", display: "flex", flexWrap: "wrap", gap: 16 }}>
                <div>
                  <div style={{ fontSize: 9, color: "#6b7280", textTransform: "uppercase", letterSpacing: 0.8 }}>Playing</div>
                  <div style={{ fontSize: 13, fontWeight: 900, color: NAVY }}>{homeField.pickSide}</div>
                </div>
                ...
                {homeField.roadRecord && <div>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 800, color: NAVY }}>{homeField.roadRecord}</div>
                </div>}
              </div>
            </Section>
          )}
            ...
            <Section title="Stadium Weather" icon="🌤️">
              {weather.isDome ? (
                <div style={{ background: "rgba(19,35,58,0.04)", borderRadius: 10, padding: "8px 12px" }}>
                  <span style={{ fontSize: 11, color: MUTED }}>🏟️ Dome / Retractable Roof — weather neutral</span>
                </div>
              ) : (
                <div style={{ background: "rgba(19,35,58,0.04)", borderRadius: 10, padding: "10px 12px" }}>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "6px 16px", marginBottom: 6 }}>
                    <div style={{ fontSize: 11, color: MUTED }}>🌡️ <b>{weather.tempF}°F</b></div>
                    <div style={{ fontSize: 11, color: MUTED }}>💨 <b>{weather.windMph}mph {weather.windDir}</b></div>
                    {weather.precipInches > 0 && <div style={{ fontSize: 11, color: MUTED }}>🌧️ <b>{weather.precipInches}"</b></div>}
                    <div style={{ fontSize: 11, color: MUTED }}>☁️ <b>{weather.cloudPct}%</b></div>
                    <div style={{ fontSize: 11, color: MUTED }}>💧 <b>{weather.humidity}%</b></div>
                  </div>
                  <div style={{ fontSize: 10, color: MUTED }}>{weather.description}</div>
                  {weather.windMph >= 15 && (
                    <div style={{ marginTop: 5, fontSize: 10, fontWeight: 700, color: "#dc2626" }}>
                      ⚡ High wind ({weather.windMph}mph) — passing game impact
                    </div>
                  )}
                  {weather.precipInches > 0.1 && (
                    <div style={{ marginTop: 3, fontSize: 10, fontWeight: 700, color: "#dc2626" }}>
                      🌧️ Precipitation — favors rushing game
                    </div>
                  )}
                  {weather.tempF < 32 && (
                    <div style={{ marginTop: 3, fontSize: 10, fontWeight: 700, color: "#3b82f6" }}>
                      ❄️ Freezing temps — ball-handling impact
                    </div>
                  )}
                </div>
              )}
          ...
          {isRunnerUp && (
            <div style={{ background: "rgba(19,35,58,0.04)", borderRadius: 8, padding: "6px 10px", marginBottom: 8 }}>
              <p style={{ fontSize: 10, color: MUTED, fontStyle: "italic", margin: 0 }}>Runner-up pick — not graded unless primary pick also fires.</p>
            </div>
          )}
    ...
    {showShareModal && (
      <div style={{
        ...
        <div style={{
          background: BG_COLOR, borderRadius: 22, width: "100%", maxWidth: 420,
          ...
          <div style={{ background: NAVY_COLOR, padding: "16px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 34, height: 34, borderRadius: 10, background: "rgba(212,168,67,0.2)",
                display: "flex", alignItems: "center", justifyContent: "center" }}>
                {isRunnerUp ? <Star size={17} style={{ color: GOLD_COLOR }} /> : <Trophy size={17} style={{ color: GOLD_COLOR }} />}
              </div>
              <div>
                <div style={{ fontSize: 9, fontWeight: 700, color: "rgba(246,241,231,0.55)", textTransform: "uppercase", letterSpacing: 1 }}>Clubhouse IQ · {sport}</div>
                <div style={{ fontSize: 14, fontWeight: 900, color: BG_COLOR }}>{label}</div>
              </div>
            </div>
            <button onClick={() => setShowShareModal(false)} style={{ background: "rgba(246,241,231,0.12)", border: "none", borderRadius: 10, width: 30, height: 30, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <X size={14} style={{ color: BG_COLOR }} />
            </button>
          </div>

          {/* Pick hero */}
          <div style={{ background: "rgba(212,168,67,0.10)", borderBottom: "1px solid rgba(19,35,58,0.10)", padding: "16px 20px", display: "flex", alignItems: "center", gap: 14 }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 22, fontWeight: 900, color: NAVY_COLOR, lineHeight: 1.1 }}>{pick.pickTeam}</div>
              <div style={{ fontSize: 11, color: MUTED, marginTop: 3 }}>{pick.awayTeam} @ {pick.homeTeam}</div>
              <div style={{ display: "flex", gap: 8, marginTop: 7, flexWrap: "wrap" }}>
                {pick.pickML != null && <span style={{ fontSize: 11, fontWeight: 700, background: NAVY_COLOR, color: GOLD_COLOR, borderRadius: 8, padding: "3px 9px" }}>ML {mlFmt(pick.pickML)}</span>}
                {pick.spread != null && <span style={{ fontSize: 11, fontWeight: 700, background: "rgba(19,35,58,0.08)", color: NAVY_COLOR, borderRadius: 8, padding: "3px 9px" }}>Spread {spreadFmt(pick.spread)}</span>}
                {pick.total != null && <span style={{ fontSize: 11, fontWeight: 700, background: "rgba(19,35,58,0.08)", color: NAVY_COLOR, borderRadius: 8, padding: "3px 9px" }}>O/U {pick.total}</span>}
              </div>
            </div>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
              <div style={{ fontSize: 36, fontWeight: 900, color: pick.grade === "A" ? "#16a34a" : pick.grade?.startsWith("B") ? "#2563eb" : GOLD_COLOR, lineHeight: 1 }}>{pick.grade}</div>
              <div style={{ fontSize: 11, fontWeight: 700, color: MUTED }}>{pick.score}/100</div>
            </div>
          </div>
          ...
          <div style={{ padding: "14px 20px", display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            {/* Injuries */}
            {((injuries.pickInjuries?.length > 0) || (injuries.oppInjuries?.length > 0)) && (
              <div>
                <div style={{ fontSize: 9, fontWeight: 800, color: MUTED, textTransform: "uppercase", letterSpacing: 1, marginBottom: 7 }}>🩹 Injury Report</div>
                <div style={{ background: "#fff", borderRadius: 12, padding: "10px 13px", border: "1px solid rgba(19,35,58,0.08)", display: "flex", flexDirection: "column", gap: 6 }}>
                  {injuries.pickInjuries?.length > 0 && (
                    <div>
                      <div style={{ fontSize: 9, color: GOLD_COLOR, fontWeight: 800, textTransform: "uppercase", marginBottom: 3 }}>{pick.pickTeam}</div>
                      {injuries.pickInjuries.map((p: string, i: number) => <div key={i} style={{ fontSize: 10, color: MUTED }}>⚠️ {p}</div>)}
                    </div>
                  )}
                  {injuries.oppInjuries?.length > 0 && (
                    <div>
                      <div style={{ fontSize: 9, color: "#6b7280", fontWeight: 800, textTransform: "uppercase", marginBottom: 3 }}>Opponent</div>
                      {injuries.oppInjuries.map((p: string, i: number) => <div key={i} style={{ fontSize: 10, color: MUTED }}>⚠️ {p}</div>)}
                    </div>
                  )}
                </div>
              </div>
            ...
            {homeField.pickSide && (
              <div>
                <div style={{ fontSize: 9, fontWeight: 800, color: MUTED, textTransform: "uppercase", letterSpacing: 1, marginBottom: 7 }}>🏟️ Home Field</div>
                <div style={{ background: "#fff", borderRadius: 12, padding: "10px 13px", border: "1px solid rgba(19,35,58,0.08)", display: "flex", gap: 16 }}>
                  <div><div style={{ fontSize: 9, color: MUTED, textTransform: "uppercase" }}>Playing</div><div style={{ fontSize: 13, fontWeight: 900, color: NAVY_COLOR }}>{homeField.pickSide}</div></div>
                  {homeField.homeRecord && <div><div style={{ fontSize: 9, color: MUTED, textTransform: "uppercase" }}>Home</div><div style={{ fontSize: 13, fontWeight: 800, color: "#16a34a" }}>{homeField.homeRecord}</div></div>}
                  {homeField.roadRecord && <div><div style={{ fontSize: 9, color: MUTED, textTransform: "uppercase" }}>Road</div><div style={{ fontSize: 13, fontWeight: 800, color: NAVY_COLOR }}>{homeField.roadRecord}</div></div>}
                </div>
              </div>
            )}
            ...
            {weather && (
              <div>
                <div style={{ fontSize: 9, fontWeight: 800, color: MUTED, textTransform: "uppercase", letterSpacing: 1, marginBottom: 7 }}>🌤️ Weather</div>
                <div style={{ background: "#fff", borderRadius: 12, padding: "10px 13px", border: "1px solid rgba(19,35,58,0.08)" }}>
                  {weather.isDome ? (
                    <span style={{ fontSize: 11, color: MUTED }}>🏟️ Dome — weather neutral</span>
                  ) : (
                    <>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: "4px 14px", marginBottom: 4 }}>
                        <span style={{ fontSize: 11, color: MUTED }}>🌡️ <b>{weather.tempF}°F</b></span>
                        <span style={{ fontSize: 11, color: MUTED }}>💨 <b>{weather.windMph}mph {weather.windDir}</b></span>
                        {weather.precipInches > 0 && <span style={{ fontSize: 11, color: MUTED }}>🌧️ <b>{weather.precipInches}"</b></span>}
                      </div>
                      <div style={{ fontSize: 10, color: MUTED }}>{weather.description}</div>
                    </>
                  )}
            ...
            {/* Edge Summary */}
            <div>
              <div style={{ fontSize: 9, fontWeight: 800, color: MUTED, textTransform: "uppercase", letterSpacing: 1, marginBottom: 7 }}>📋 Edge Summary</div>
              <div style={{ background: "#fff", borderRadius: 12, padding: "10px 13px", border: "1px solid rgba(19,35,58,0.08)", display: "flex", flexDirection: "column", gap: 5 }}>
                {(pick.reasons ?? []).map((reason: string, i: number) => {
                  const isNeg = /caution|injury|weather|wind|rain|snow/i.test(reason);
                  return (
                    <div key={i} style={{ display: "flex", gap: 7, alignItems: "flex-start" }}>
                      <div style={{ width: 16, height: 16, borderRadius: 8,
                        background: isNeg ? "rgba(239,68,68,0.10)" : "rgba(34,197,94,0.10)",
                        display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 1 }}>
                        <span style={{ fontSize: 9, fontWeight: 900, color: isNeg ? "#dc2626" : "#16a34a" }}>{isNeg ? "✕" : "✓"}</span>
                      </div>
                      <span style={{ fontSize: 11, color: MUTED, lineHeight: 1.4 }}>{reason}</span>
                    </div>
                  );
            ...
            {/* Footer */}
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: 6, borderTop: "1px solid rgba(19,35,58,0.08)" }}>
              <span style={{ fontSize: 10, color: "#6b7280" }}>clubhouseiqbets@gmail.com</span>
              <span style={{ fontSize: 10, fontWeight: 700, color: GOLD_COLOR }}>Clubhouse IQ 🏈</span>
            </div>
          </div>
        </div>
      </div>
